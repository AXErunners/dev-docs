...
axe even if this parameter is set to `1` or higher.{% endcapture %}
...
- n: "{{DEPTH}} →<br>`type`"
  t: "number (int)"
  p: "Required<br>(exactly 1)"
  d: "*Added in Axe Core 0.13.0.0*<br><br>The transaction format type"
...
- n: "{{DEPTH}} → → →<br>`value`"
  ...
  d: "The number of Axe paid to this output.  May be `0`.<br><br>Only present if `spentindex` enabled"

- n: "{{DEPTH}} → → →<br>`valueSat`"
  t: "number (haks)"
  p: "Optional<br>(exactly 1)"
  d: "The number of haks paid to this output.  May be `0`.<br><br>Only present if `spentindex` enabled"
...
- n: "{{DEPTH}} → → →<br>`value`"
  t: "number (Axe)"
  p: "Required<br>(exactly 1)"
  d: "The number of Axe paid to this output.  May be `0`"

- n: "{{DEPTH}} → → →<br>`valueSat`"
  t: "number (haks)"
  p: "Required<br>(exactly 1)"
  d: "The number of haks paid to this output.  May be `0`"
...
- n: "{{DEPTH}} →<br>`extraPayloadSize`"
  t: "number (int)"
  p: "Optional<br>(0 or 1)"
  d: "*Added in Axe Core 0.13.0.0*<br><br>Size of the DIP2 extra payload. Only present if it's a DIP2 special transaction"

- n: "{{DEPTH}} →<br>`extraPayload`"
  t: "string (hex)"
  p: "Optional<br>(0 or 1)"
  d: "*Added in Axe Core 0.13.0.0*<br><br>Hex encoded DIP2 extra payload data. Only present if it's a DIP2 special transaction"
{% endcapture %}
...
{% assign reindexNote="Note: if you begin using `txindex=1` after downloading the block chain, you must rebuild your indexes by starting Axe Core with the option  `-reindex`.  This may take several hours to complete, during which time your node will not process new blocks or transactions. This reindex only needs to be done once." %}
