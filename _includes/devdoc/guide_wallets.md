...
A Axe wallet can refer to either a wallet program or a wallet file.
Wallet programs create public keys to receive haks and use the
corresponding private keys to spend those haks. Wallet files
...
Permitting receiving and spending of haks is the only essential
...
program distributing public keys in order to receive haks and
another program signing transactions spending those haks.
...
2. (Online) Install the wallet software on another device, this one
   ...
   public keys to receive payment. When ready to spend haks, fill in
...
transactions before signing, the user's haks are safe even if the
...
Axe wallets at their core are a collection of private keys. These
...
Private keys are what are used to unlock haks from a particular address. In Axe, a private key in standard format is simply a 256-bit number, between the values:

0x01 and 0xFFFF FFFF FFFF FFFF FFFF FFFF FFFF FFFE BAAE DCE6 AF48 A03B BFD2 5E8C D036 4140, representing nearly the entire range of 2<sup>256</sup>-1 values. The range is governed by the secp256k1 ECDSA encryption standard used by Axe.
...
In order to make copying of private keys less prone to error, [Wallet Import Format][/en/glossary/wallet-import-format]{:#term-wallet-import-format}{:.term} may be utilized. WIF uses base58Check encoding on an private key, greatly decreasing the chance of copying error, much like standard Axe addresses.
...
Axe ECDSA public keys represent a point on a particular Elliptic
...
shows such a point on the elliptic curve used by Axe,
...
compressed public keys are the default in Axe Core and are the
recommended default for all Axe software.
...
For this reason, Bitcoin Core (and Axe Core) uses several different identifier bytes to
...
sum divided by a global constant used by all Axe software (*p*):
...
(Axe developers typically use the ASCII apostrophe rather than
...
Loose-Key wallets, also called "Just a Bunch Of Keys (JBOK)", are a form of wallet that originated from the Bitcoin Core client wallet. The Axe Core client wallet creates 1000 private key/public key pairs automatically via a Pseudo-Random-Number Generator (PRNG) for later use.
