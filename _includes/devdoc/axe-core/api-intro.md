...
{% endcomment %}
{% assign filename="_includes/devdoc/axe-core/api-intro.md" %}
...
Axe Core RPCs accept and return the byte-wise reverse of computed
...
However, Axe Core's RPCs use the byte-wise reverse for hashes, so if you
...
Axe Core's use of hashes (which are byte arrays in C++) as integers
...
other Axe software, this reversal of hashes has become the standard
