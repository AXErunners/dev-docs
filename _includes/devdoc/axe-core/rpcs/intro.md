...
{% endcomment %}
{% assign filename="_includes/devdoc/axe-core/rpcs/intro.md" %}
...
Axe Core provides a remote procedure call (RPC) interface for various
administrative tasks, wallet operations, and queries about network and block
chain data.

If you start Axe Core using `axe-qt`, the RPC interface is disabled by
default. To enable it, set `server=1` in `axe.conf` or supply the `-server`
argument when invoking the program. If you start Axe Core using `axed`,
...
`axe.conf` or by supplying the `-rpcpassword` program argument. Optionally a
username can be set using the `rpcuser` configuration value. See the [Examples
Page][devexamples] for more information about setting Axe Core configuration
values.
...
scratch. Axe Core also ships with its own compiled C++ RPC client,
`axe-cli`, located in the `bin` directory alongside `axed` and
`axe-qt`. The `axe-cli` program can be used as a command-line interface
(CLI) to Axe Core or for making RPC calls from applications written in
...
| → <br>`jsonrpc`      | number (real)   | Optional<br>(0 or 1)        | Version indicator for the JSON-RPC<!--noref--> request. Currently ignored by Axe Core.
...
| → <br>`params`       | object           | Optional<br>(0 or 1)        | Starting from Axe Core 0.12.3 / Bitcoin Core 0.14.0 (replaces the params array above) An object containing named parameter values for the RPC. May be an empty object or omitted for RPC calls that don’t have any required parameters.
...
* "Type" is the JSON data type and the specific Axe Core type.
...
The command to send this request using `axe-cli` is:

{% highlight bash %}
axe-cli getblockhash 0
{% endhighlight %}

The command to send this request using `axe-cli` with named parameters is:

{% highlight bash %}
axe-cli -named getblockhash height=0
{% endhighlight %}
...
Note: In order to minimize its size, the raw JSON response from Axe Core
doesn't include any extraneous whitespace characters. Here we've added
whitespace to make the object more readable. Speaking of which, `axe-cli`
...
Continuing with the example above, the output<!--noref--> from the `axe-cli`
...
If there's an error processing a request, Axe Core sets the `result` field
...
If `axe-cli` encounters an error, it exits with a non-zero status code and
...
request batching may provide significant performance gains. The `axe-cli`
...
available RPC calls will be given as `axe-cli` commands:

{% highlight text %}
axe-cli [options] <method name> <param1> <param2> ...
{% endhighlight %}
