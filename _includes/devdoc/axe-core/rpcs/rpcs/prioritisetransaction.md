...
{% endcomment %}
{% assign filename="_includes/devdoc/axe-core/rpcs/rpcs/prioritisetransaction.md" %}
...
- n: "Fee"
  t: "number (int)"
  p: "Required<br>(exactly 1)"
  d: "**Warning:** this value is in haks, not Axe<br><br>If positive, the virtual fee to add to the actual fee paid by the transaction; if negative, the virtual fee to subtract from the actual fee paid by the transaction.  No change is made to the actual fee paid by the transaction"
...
{% highlight bash %}
axe-cli -testnet prioritisetransaction \
