...
The Axe network protocol allows full nodes
...
clients also use the Axe network protocol to connect to full nodes.

Consensus rules do _not_ cover networking, so Axe programs may use
...
To provide practical examples of the Axe peer-to-peer network, this
section uses Axe Core as a representative full node and [AxeJ][]
...
hardcoded into Axe Core. The response to the lookup should include one or more
...
The DNS seeds are maintained by Axe community members: some of them
...
peer discovery. Axe Core keeps a record of known peers in a
...
<!-- reference for "Bitcoin Core...11 seconds" below:
     ...
     https://github.com/axerunners/axe/commit/2e7009d67b862cf822a1c70e181de6af659a3096
     -->

Axe Core tries to strike a balance between minimizing delays
and avoiding unnecessary DNS seed use: if Axe Core has entries in
...
Axe Core also includes a hardcoded list of IP addresses and port numbers to
several dozen nodes which were active around the time that particular
version of the software was first released. Starting with Axe Core 0.12.3,
...
consensus rules (reliable). Axe Core will start attempting to connect to these
nodes if none of the DNS seed servers have responded to a query within 60
seconds, providing an automatic fallback option.

As a manual fallback option, Axe Core also provides several
...
**Resources:** [Axe Seeder][], the program run by several of the
seeds used by Axe Core. The Axe Core [DNS Seed Policy][]. The hardcoded list
 of IP addresses used by Axe Core is generated using the [makeseeds script][].
...
Axe Core uses the IBD method any time the last block on its local
best block chain has a block header time more than 24 hours in the past.
Axe Core will also perform IBD if its local best block chain is
...
Axe Core (up until version 0.12.0.x) uses a simple initial block download (IBD)
...
* **Speed Limits:** All requests are made to the sync node, so if the
  ...
  download speeds.  Note: if the sync node goes offline, Axe Core
...
* **Download Restarts:** The sync node can send a non-best (but
  ...
  over again from a different node. Axe Core ships with several
...
headers-first IBD method used in Axe Core 0.12.0.x.
...
Axe Core 0.12.0 uses an initial block download (IBD) method called
...
1. **Download More Headers:** the IBD node can send another `getheaders`
   ...
   of downloaded data.
    ...
    connects to at least one honest peer; Axe Core will continue to
...
2. **Download Blocks:** While the IBD node continues downloading
   ...
   single sync node.
    ...
    To spread the load between multiple peers, Axe Core will only
    ...
    Axe Core will request a maximum of 128 blocks simultaneously
    during IBD (the same maximum number that blocks-first Axe Core
    requested from its sync node).

![Simulated Headers-First Download Window](/img/dev/en-headers-first-moving-window.svg)

Axe Core's headers-first mode uses a 1,024-block moving download
...
time Axe Core is ready to validate it, Axe Core will wait a
minimum of two more seconds for the stalling node to send the block. If
the block still hasn't arrived, Axe Core will disconnect from the
...
By default, Axe Core broadcasts blocks using standard block relay,
...
Because unconfirmed transactions have no permanent status in Axe,
Axe Core stores them in non-persistent memory, calling them a memory
...
include them. This is the case in Axe Core, which removes stale
...
*Removed in Bitcoin Core 0.13.0. Retained in Axe Core (see [PR1326](https://github.com/axerunners/axe/pull/1326)).*

In case of a bug or attack, the Axe Core developers can issue an alert via the
Axe network. Alerts will be displayed by the Axe Core UI. Users can also
check the error field of the `getinfo` RPC results to get currently active
alerts for their specific version of Axe Core or use the `-alertnotify` command
