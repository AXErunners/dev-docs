...
As illustrated below, solo miners typically use `axed` to get new
transactions from the network. Their mining software periodically polls
`axed` for new transactions using the `getblocktemplate` RPC, which
...
block and sends the completed block to `axed` to be broadcast to the network
...
`axed`. Using one of the methods discussed later, each miner's mining
...
The simplest and earliest method was the now-deprecated Axe Core
...
An improved method is the Axe Core `getblocktemplate` RPC. This
provides the mining software with much more information:

1. The information necessary to construct a coinbase transaction
   paying the pool or the solo miner's `axed` wallet.

2. A complete dump of the transactions `axed` or the mining pool
...
Like all `axed` RPCs, `getblocktemplate` is sent over HTTP. To
