...
Contracts are transactions which use the decentralized Axe system to enforce
financial agreements.
Axe contracts can often be crafted to minimize dependency on outside
...
The following subsections will describe a variety of Axe contracts
...
A simple contract could say that Charlie will spend haks to an
...
contract][/en/glossary/escrow-contract]{:#term-escrow-contract}{:.term}. Charlie spends his haks
...
who should get the haks if there's a dispute.
...
redeem script to create a P2SH redeem script and pays the haks to it. Bob
...
of the haks to Bob's public key and one that spends the remaining
...
agree to, such as spending all the haks to herself, Bob and Charlie
can find a new arbitrator and sign a transaction spending the haks
...
to expire before further spending his milliaxe. Bob hasn't actually
...
Now, when Alice does some work worth 1 milliaxe, she asks Bob to create
and sign a new version of the refund transaction.  Version two of the
transaction spends 1 milliaxe to Alice and the other 99 back to Bob; it does
...
two transactions, Alice could hold Bob's 100 milliaxe hostage even if she
hadn't done any work.

For larger payments, Axe transaction fees are very low as a
...
<!-- axej _may_ support the bitcoinj micropayment functions, but has not been tested
**Resource:** The [axej][] Java library
...
can each easily track those haks to learn what Bitcoin
...
where she has spent her haks and how many she has left, so she
...
agree to transfer haks between each other so no one besides them
can reliably determine who controls which haks.  But they're faced
with a dilemma: who transfers their haks to one of the other two
...
of them can steal the others' haks.
...
Outputs (UTXOs) for 100 milliaxe they can spend. They then each generate
...
the transaction to the peer-to-peer network, mixing all of the milliaxe in
...
The complete history of Alice's haks is still in the block chain,
so a determined investigator could talk to the people AnonGirl
CoinJoined with to find out the ultimate origin of her haks and
...
amount of haks to pay the transaction fee.  An alternative
technique, purchaser CoinJoin, can actually save them haks and
improve their privacy at the same time.

AnonGirl waits in the IRC chatroom until she wants to make a purchase.
She announces her intention to spend haks and waits until someone
...
saving each one of them a tiny amount of haks.
