...
The block chain provides Axe's public ledger, an ordered and timestamped record
of transactions. This system is used to protect against double spending
and modification of previous transaction records.

Each full node in the Axe network independently stores a block chain
...
the consensus rules used by Axe Core.
...
Transactions are also chained together. Axe wallet software gives
the impression that haks are sent from and to wallets, but
Axe value really moves from transaction to transaction. Each
transaction spends the haks previously received in one or more earlier
...
spend---an attempt to spend the same haks twice.
...
fee][/en/glossary/transaction-fee]{:#term-transaction-fee}{:.term} by the Axe
[miner][/en/glossary/mining]{:#term-miner}{:.term} who creates the block containing that
transaction.
For example, in the illustration above, each transaction spends 10,000 haks
...
Axe requires that each block prove a significant amount of work was invested in
...
The [proof of work][/en/glossary/proof-of-work]{:#term-proof-of-work}{:.term} used in Axe
...
threshold.
Axe assumes a linear probability that the lower it makes the target threshold, the more hash attempts (on average) will need to be tried.
...
This method of calculating difficulty (Dark Gravity Wave) was authored by Axe
creator Evan Hakield to fix exploits possible with the previously used
Kimoto Gravity Well difficulty readjustment algorithm. For additional detail,
reference this [Official Documentation Dark Gravity Wave page](https://axerunners.atlassian.net/wiki/spaces/DOC/pages/1146926/Dark+Gravity+Wave).
...
propagate a modified block as the entire Axe network expended
...
Any Axe miner who successfully hashes a block header to a value
...
by their [block height][/en/glossary/block-height]{:#term-block-height}{:.term}---the number of blocks between them and the first Axe
...
Axe Core includes code that detects a hard fork by looking at block
...
use the current consensus rules. Axe Core reports this situation
