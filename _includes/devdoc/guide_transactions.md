...
Transactions let users spend haks. Each transaction is constructed
...
not exist. Coinbase transactions can only be created by Axe miners
...
The figure above shows the main parts of a Axe transaction. Each
transaction has at least one input and one output. Each [input][/en/glossary/input]{:#term-input}{:.term} spends the
haks paid to a previous output. Each [output][/en/glossary/output]{:#term-output}{:.term} then waits as an Unspent
Transaction Output (UTXO) until a later input spends it. When your
Axe wallet tells you that you have a 10,000 hak balance, it really
means that you have 10,000 haks waiting in one or more UTXOs.

Each transaction is prefixed by a four-byte [transaction version number][]{:#term-transaction-version-number}{:.term} which tells
Axe peers and miners which set of rules to use to validate it.  This
...
amount in haks which it pays to a conditional pubkey script. Anyone
who can satisfy the conditions of that pubkey script can spend up to the
amount of haks paid to it.
...
type. [P2PKH][/en/glossary/p2pkh-address]{:#term-p2pkh}{:.term} lets Alice spend haks to a typical Axe address,
and then lets Bob further spend those haks using a simple
...
first transaction. Axe uses the Elliptic Curve Digital Signature Algorithm (ECDSA) with
...
sent encoded as Axe [addresses][/en/glossary/address]{:#term-address}{:.term}, which are base58-encoded strings
...
into another format, such as a QR code containing a `axe:`
URI.
...
recipient spend this transaction's output, and the amount of haks to
...
broadcasts the transaction to Axe miners through the peer-to-peer
...
Axe addresses and there was no standard way to communicate them
...
hash---so it can be transformed into the standard Axe address format
...
by nodes not using the default Axe Core settings. If they are
included in blocks, they will also avoid the IsStandard test and be
processed.

Besides making it more difficult for someone to attack Axe for
...
As of Axe Core 0.12.2, the standard pubkey script types are:
...
Axe Core 0.11.x will, by default, relay and mine null data
transactions with up to 40 bytes in a single data push and only one null
data output that pays exactly 0 haks:
...
The `-datacarriersize` Axe Core configuration option allows you to
...
and miners using the default Axe Core settings will neither
...
standard transactions which make the haks sent to them unspendable.
...
* The transaction must not include any outputs which receive fewer than
  1/3 as many haks as it would take to spend it in a typical input.
  That's currently 5460 haks for a
  P2PKH or P2SH output on a Axe Core node with the default relay fee.
  Exception: standard null data outputs must receive zero haks.
...
* [`SIGHASH_NONE`][/en/glossary/sighash-none]{:#term-sighash-none}{:.term} signs all of the inputs but none of the outputs,
  allowing anyone to change where the haks are going unless other
...
* `SIGHASH_ALL|SIGHASH_ANYONECANPAY` signs all of the outputs but only
  ...
  inputs, so anyone can contribute additional haks but they cannot
  change how many haks are sent nor where they go.
...
the haks without consulting the `NONE` signer---but nobody else can
...
(Called nLockTime in the Axe Core source code.)
...
Previous versions of Axe Core provided a feature which prevented
...
Axe Core) can still disable the time lock, so if you want to use
...
Axe miner, as explained in the [block chain section][section block chain], and
...
which spend haks that have not moved for a long time.

These "priority" transaction can be often exempt from the normal fee
requirements. As of Axe Core 0.12.2, 10 KB of each block are reserved for these
...
As of Axe Core 0.12.2.x, a [minimum fee][/en/glossary/minimum-relay-fee]{:#term-minimum-fee}{:.term}(1,000 haks following [DIP1][] activation) is required to
...
[Change outputs][/en/glossary/change-address]{:#term-change-output}{:.term} are regular outputs which spend the surplus haks
...
Axe addresses (hashed public keys) as static payment addresses,
other people can easily track the receiving and spending habits of that
person, including how many haks they control in known addresses.
...
1. Unique (non-reused) P2PKH and P2SH addresses protect against the first
   ...
   first time haks sent to those addresses are spent, so attacks
...
fixed URI to which payments should be sent, please use `axe:` URIs as defined
...
None of Axe's signature hash types protect the signature script, leaving
...
This isn't a problem for most Axe transactions which are designed to
...
Axe Core 12.3 implemented [BIP-147: Dealing with dummy stack element malleability](https://github.com/bitcoin/bips/blob/master/bip-0147.mediawiki)
...
Transaction malleability also affects payment tracking.  Axe Core's
...
reference [this blog post](https://blog.axe.org/segwit-lighting-rbf<!--noref-->-in-axe-9536868ca861)
by one of the Axe Core developers.
