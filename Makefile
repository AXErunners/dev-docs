...
## have documented
	$S for f in _includes/devdoc/axe-core/rpcs/rpcs/*.md ;\
	do grep -q "\[$$( grep '^##### ' $$f | sed 's/^##### *\([a-zA-Z0-9]*\).*/\1/')\]\[" _includes/devdoc/axe-core/rpcs/quick-reference.md \
	|| echo 'missing summary for '$$f', you need to add the summary to _includes/devdoc/axe-core/rpcs/quick-ref.md and run make manual-updates' \
	; done | eval $(ERROR_ON_OUTPUT)
